...
# through scripts/day2/user-lifecycle.sh (`ggadmin-user set-password
...
#
# Run as root: sudo bash set-service-credentials.sh <account> [<account> ...]
...
# or nothing at all (no arguments defaults to `all`):
